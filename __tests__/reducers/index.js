...
describe('Karaoke App', () => {
  ...
  describe('lyricChangeReducer', () => {
    ...
    it('should update state on receive song', ()=> {
      const action = actions.receiveSong('kiss', 'prince', 1, ['you don\'t have have to be beautiful', 'to turn me on']);
      const newObject = {
        isFetching: false,
        title: action.title,
      ...
      expect(lyricChangeReducer(inititalState.songsById, action)[action.songId]).toEqual(newObject);
    });
  });
